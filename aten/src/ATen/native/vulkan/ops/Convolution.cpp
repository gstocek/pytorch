...
c10::intrusive_ptr<Conv1dPackedContext> create_conv1d_context(
    ...
    std::vector<int64_t>&& dilation,
    const int64_t groups) {
  return c10::make_intrusive<Conv1dPackedContext>(Conv1dPackedContext(
      weight,
      bias,
      stride,
      padding,
...
Tensor convolution1d(
    ...
    const IntArrayRef dilation,
    const int64_t groups) {
  Conv1dPackedContext conv1d_context = Conv1dPackedContext(
      weight,
      bias,
      stride,
      padding,
...
Tensor run_conv1d_context(
  ...
  return conv1d::run_conv1d_context_impl(input, weight, bias_opt, stride, padding, dilation, groups);
}
